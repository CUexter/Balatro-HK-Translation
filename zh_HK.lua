return {
	...
	mods = {
		mp = {
			...
			failed = " 失敗咗 ",
			defeat_enemy = "贏咗對手",
			total_lives_lost = "總共損失命數 (每條命 $4)",
			attrition_name = "消耗戰",
			attrition_desc = "每個Boss 回合都係玩家之間嘅比賽， 得分較低嘅玩家會損失一條命",
			draft_name = "選拔戰",
			draft_desc = "雙方玩家會玩三次正常嘅底注， 然後佢哋會玩一次底注， 每回合分數較低嘅人會損失一條命",
			royale_name = "大亂鬥",
			royale_desc = "好似消耗戰咁，但係有八個玩家同埋每人都只有一條命",
			vanilla_plus_name = "經典+",
			vp_desc = "第一個輸嘅人就輸，冇PVP嘅盲注",
			headup_name = "先拔頭籌",
			hu_desc = "雙方玩家玩完第一次底注，然後必須打出高過對方上一手嘅分數，否則就輸 ",
			...
			discord_msg = "你入到去可以同我哋匯報搵到嘅bugs同埋搵人陪你玩",
			enter_to_save = "按Enter儲存",
			in_lobby = "喺大廳",
			connected = "服務已連接",
			warn_service = "警告：無法找到多人伺服器",
			set_name = "可以係主選單設定自己嘅名(Mods > Multiplayer)",
			start = "開始",
			wait_for = "等緊",
			host_start = "大廳主開始",
			players = "其他玩家",
			...
			view_code = "查看代碼",
			leave = "離開",
			opts_only_host = "只有大廳主能夠改呢啲設定",
			opts_cb_money = "死左會俾返啲錢東山再起",
			opts_no_gold_on_loss = "輸左回合會冇咗果個盲注嘅錢",
			opts_death_on_loss = "如果非PvP回合輸左會損失一條命",
			opts_start_antes = "一開始正常底注數目",
			opts_diff_seeds = "玩家有唔同嘅SEED",
			opts_lives = "命數",
			opts_gm = "模式修改效果",
			bl_pvp_name = "你嘅對手",
			bl_pvp_desc1 = "面對其他玩家，",
			bl_pvp_desc2 = "最多分數嘅人贏",
			bl_or = "",
			bl_life = "一係生",
			bl_death = "一係死",
			lobby = "大廳",
